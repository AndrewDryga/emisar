...
description: Every successful main push publishes its exact tested portal image; production planning has no stale-image fallback, and health reports the compiled source revision.
subsystem: infra
...
`/readyz` therefore report the Git revision compiled into the image, and
