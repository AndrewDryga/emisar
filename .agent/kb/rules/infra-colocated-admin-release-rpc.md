...
**Rule.** When a private first-party runner is intentionally trusted with control of the portal host, its fixed action script invokes one bounded Elixir context entrypoint through the colocated release's `bin/emisar rpc`. A one-off pack uses the runner's existing argv contract; it never adds pack-specific metadata or behavior to the client-shipped runner. Encode argv into inert literals in the fixed RPC expression instead of reading `docker exec --env` variables: release RPC evaluates inside the already-running BEAM, whose environment is not the release client's environment. Do not add a private HTTP endpoint, callback credential, local listener, or custom transport just to enter the same BEAM.
...
```sh
action_id=$(printf %s emisar.admin.account.show | base64 | tr -d '\n')
docker exec emisar /app/bin/emisar rpc \
  "Emisar.Admin.execute(Base.decode64!(\"$action_id\"), [])"
```
