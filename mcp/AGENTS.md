...
- **Three source files, stdlib only** (the `internal/attest` package is the only non-stdlib import, and it's in-tree). A new *external* dep needs a strong, stated reason.
  - **`main.go`** (~590 lines) — the zero-protocol forwarder: scan stdin, forward, key rotation, help/version. This is the part that "does nothing the portal couldn't."
