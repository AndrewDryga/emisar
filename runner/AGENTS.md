...
`install.sh` shell and behavior harness. The harness runs all nine
...
- Toolchain is **Go 1.26.5** (`go.work`); deps are deliberately few (`coder/websocket`, `spf13/cobra`, `oklog/ulid`, `yaml.v3`, and `santhosh-tekuri/jsonschema/v6` for typed `output.schema` result validation in `internal/outputschema`). A new dependency on the host runner is new attack surface — justify it in one sentence, and prefer the stdlib.
