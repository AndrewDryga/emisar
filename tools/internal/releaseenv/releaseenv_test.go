...
func TestProfiles(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			profile := profiles[test.environment]
			if err := validateEnvironment(profile, []byte(test.environmentJSON)); err != nil {
				t.Fatal(err)
			}
...
func TestVerifyFetchesTheExactTypedPolicySet(t *testing.T) {
	var paths []string
	fetch := func(_ context.Context, path string) ([]byte, error) {
		paths = append(paths, path)
		switch len(paths) {
		case 1:
			return []byte(publicReleasesEnvironment), nil
		case 2:
			return []byte(publicReleasePolicies), nil
		default:
			return nil, fmt.Errorf("unexpected fetch %q", path)
		}
	}
	var out bytes.Buffer
	if err := Verify(t.Context(), "AndrewDryga/emisar", "public-releases", fetch, &out); err != nil {
		t.Fatal(err)
	}
	wantPaths := []string{
		"repos/AndrewDryga/emisar/environments/public-releases",
		"repos/AndrewDryga/emisar/environments/public-releases/deployment-branch-policies?per_page=100",
	}
	if !reflect.DeepEqual(paths, wantPaths) {
		t.Fatalf("paths = %v, want %v", paths, wantPaths)
	}
	if out.String() != "verified: AndrewDryga/emisar/public-releases matches its release policy\n" {
		t.Fatalf("output = %q", out.String())
	}
}

func TestEnvironmentRejectsWeakenedOrMalformedProtection(t *testing.T) {
	tests := []struct {
		name        string
		environment string
		payload     string
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			profile := profiles[test.environment]
			err := validateEnvironment(profile, []byte(test.payload))
...
func TestDeploymentBranchPoliciesMustMatchExactly(t *testing.T) {
	want := profiles["public-releases"].customPolicies
	tests := []struct {
		name    string
		payload string
		want    string
