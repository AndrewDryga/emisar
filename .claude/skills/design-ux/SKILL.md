...
- Use `product-manager` when scope, persona, or the smallest valuable flow is
  unclear.
- Use `design-creative-director` when the product app needs to align with an approved
...
- Use `design-frontend` for HEEx/Tailwind/CoreComponents execution.
- Use `design-interface-polish` for the micro-craft polish pass after the
  screen renders.
...
7. **Align with the direction.**
   If `design-creative-director` has set an art direction, adapt its principles to the
...
- Does the result align with any `design-creative-director` direction without making the
  app noisy?
...
- <CoreComponents/LiveTable/design-frontend handoff details>
```
