...
for what is really one new function — use `/elixir-context-fn` instead.
...
6. **Context** — `lib/emisar/<context>.ex` — the public API (§1, IL-1…IL-5); `@doc` each public fn with its contract — permission + return shape, not narration — and the `@moduledoc` names it the authorization boundary for the domain (`.agent/kb/rules/elixir-doc-contract.md`).
