...
6. **Context** — `lib/emisar/<context>.ex` — the public API (§1, IL-1…IL-5); `@doc` each public fn with its contract — permission + return shape, not narration — and the `@moduledoc` names it the authorization boundary for the domain (`.agent/rules/elixir-doc-contract.md`).
