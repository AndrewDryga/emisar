...
grep -Fq 'http://127.0.0.1:4000/install.sh' "$rendered"
grep -Fq '/bin/bash "$installer"' "$rendered"
grep -Fq "required_packs='linux-core debugging systemd-deep cloud-init docker firewall nic time-sync elixir-beam'" "$rendered"
...
grep -Fq 'test -r /var/lib/emisar-admin-runner/packs/emisar-admin/scripts/callback.sh' "$rendered"
grep -Fq '"$runner" pack install "$pack"' "$rendered"
grep -Fq '"$runner" pack list --packs-dir /var/lib/emisar-admin-runner/packs' "$rendered"
...
grep -Fq -- '- /var/lib/emisar-admin-runner/packs' "$rendered"
grep -Fq -- '- "beam.*"' "$rendered"
