...
ver=$(curl -fsS $K "$PURE_URL/api/api_version" | grep -oE '2\.[0-9]+' | sort -t. -k2 -n | tail -1)
[ -n "$ver" ] || ver=2.2

# Exchange the API token for a session token (api-token in -> x-auth-token out).
sess=$(printf 'api-token: %s\n' "${PURE_API_TOKEN:-}" |
	curl -fsS $K -D - -o /dev/null -X POST -H @- "$PURE_URL/api/$ver/login" |
...
printf 'x-auth-token: %s\n' "$sess" | curl -fsS $K -G -H @- "$@" "$PURE_URL/api/$ver$path"
