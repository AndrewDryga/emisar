...
- **Shared:** [Docker build metadata stays after reusable layers](.agent/rules/shared-docker-build-metadata-stays-late.md) — consume commit SHAs, build IDs, timestamps, and other per-build metadata only in the final image layer so stable toolchain, dependency, asset, and release layers remain cacheable.
