...
3. **`.agent/kb/README.md`'s index** — the durable-knowledge routing table. Open a descriptive card only when your task touches its subsystem, then read the touched project's indexed `kb/rules/`; never bulk-load the KB.
...
- **`kb/`** — the single home for durable knowledge. Cards directly under it are DESCRIPTIVE current-system facts: when implementation changes, its affected card changes too. `kb/rules/` contains NORMATIVE constraints: a change that contradicts one is wrong even if its descriptive card is updated. When both apply, split and link them instead of duplicating the explanation. Read `kb/README.md` for the decision test, card format, and on-demand reading protocol.
