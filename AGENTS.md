...
- **Shared:** [human development tooling is not agent state](.agent/rules/shared-human-dev-tooling-is-not-agent-state.md) — put commands used by people and agents under `dev/`, share one dependency Compose topology, and reserve `.agent/scripts/` for agent-only enforcement and orchestration.
