...
- **Shared:** [human development tooling is not agent state](.agent/rules/shared-human-dev-tooling-is-not-agent-state.md) — expose shared commands through `dev/run`, keep reusable implementations under `tools/`, share one dependency Compose topology, and keep `.agent/` as configuration/state rather than another command surface.
