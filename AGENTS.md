...
- **Shared:** [human development tooling is not agent state](.agent/kb/rules/shared-human-dev-tooling-is-not-agent-state.md) — expose shared commands through `./run`, keep reusable implementations under `tools/`, share one application dependency topology, colocate integration SUTs with their owning tests, keep disposable screenshots with their task, give generators owned output subtrees, and keep `.agent/` as configuration/state rather than another command surface.
