...
- **Shared:** [human development tooling is not agent state](.agent/kb/rules/shared-human-dev-tooling-is-not-agent-state.md) — expose shared commands through `./run`, keep reusable implementations under `tools/`, keep dev-only images and fixtures under `dev/`, share one dependency Compose topology, keep disposable screenshots with their task, give generators owned output subtrees, and keep `.agent/` as configuration/state rather than another command surface.
