...
| Naked status note (canvas) | `<.status_note icon= tone= title= primary>` — toned icon lead + title + body, NO box, for a note ABOUT the surface (credential warning, posture fact, reach statement); `primary` = the page's strongest status voice (semibold). `minted_note` (agents) is its one thin wrapper. **Deliberate singleton left inline:** the install wizard's dot-led "Waiting for a runner to connect" block (live PING-dot lead + nested watchdog troubleshooting — live-status semantics, not a static note); extract a dot-lead variant only when a SECOND dot-led status block appears | a hand-rolled `flex items-start gap-3` + `mt-0.5 h-4 w-4` icon + title/body divs (agents / runner-detail / install-wizard each had a private copy before the 2026-07-05 sweep) |
| Transient event block | `<.event_block icon= tone={:amber\|:rose} title=>` + `:body` slot + payload in the default slot — the toned icon caps a quiet spine binding note + artifact + actions into one unit on a page whose main content is something else. `:amber` = pending on someone/something (rotation reveal, approval hold, runner offline); `:rose` = a dead outcome (cancelled, errored) | a `border-l-2` wrapper, a boxed banner/`callout` for a run-state fact, or three floating elements for an action result on a list page |
...
| Radio choice-card group | `<.choice_cards name= value= columns=>` + `:card value= icon= title=` slots — sr-only radio, NEUTRAL selection ring + check (a chosen risky option never wears the safe hue). **Any pick-one comparison grid whose options carry their own CTAs (billing plan cards) reuses the RECIPE even when it can't be a radio group:** `rounded-lg p-* ring-1`, current/selected = `bg-white/[0.04] ring-white/25`, rest = `bg-black/20 ring-zinc-800` — never a bespoke card wash | a hand-rolled `<label>` + radio card list, per-page selected-class helpers, or a `border-zinc-800/70 bg-zinc-950/40` comparison card |
