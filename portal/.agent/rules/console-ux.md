...
| **Wizard** | `:form` (`:detail` with a rail) | one guided surface · one primary action per step · live wait states with escalating troubleshooting · page-advance keyed to the specific entity it created · a wizard carrying a supporting rail (trust facts, resources) goes two-column at `lg` — task left, reading right behind a vertical hairline — and runs `:detail`, never a `:form` tower beside dead space | RunnerInstall |
