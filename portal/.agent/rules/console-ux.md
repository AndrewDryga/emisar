...
| **Detail** | `:table` — the SAME width as the list it's reached from, so the header never jumps (runner detail + run detail precedents; approval/audit details still `:detail` pending their canvas pass) | `detail_header` (back · entity title · `:actions`) · NAKED meta row on the canvas, Status first (one flex row at sm+, 2-col grid on phones — no `meta_strip` island) · conditional callout stack (tight `space-y-3` group inside the page's `space-y-12` wrapper) · canvas sections (`section_header`) · artifacts as `code_panel`-framed boxes · danger zones last | RunDetail, RunnerDetail |
