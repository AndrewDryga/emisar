...
- For a **self-service** action, read the user from `subject.actor`; don't also accept it as a separate arg. That actor match establishes ownership, not personal authority. Shared personal profile, email and session controls call `Subject.ensure_personal_user/1` before database, throttle or mail work: only a User authenticated by a personal email link qualifies. SSO, missing provenance, workspace roles and IdP MFA assertions do not qualify; there is no admin/override bypass.
