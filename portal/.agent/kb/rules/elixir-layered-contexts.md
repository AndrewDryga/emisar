...
- For a **self-service** action, read the user from `subject.actor`; don't also accept it as a separate arg. That actor match establishes ownership, not personal authority. Shared personal profile, email and session controls call `Subject.ensure_personal_user/1` before database, throttle or mail work: the exact browser must retain unexpired independent personal email-link proof. SSO alone, missing provenance, workspace roles and IdP MFA assertions do not qualify; there is no admin/override bypass. SSO step-up preserves existing personal proof only until its original expiry.
- Independently factor-proved local MFA enrollment, step-up, disable and recovery remain available to an SSO session under their own credential gates. Proving a local factor does not rewrite SSO provenance or unlock personal profile/session/email controls. Do not blanket-gate MFA helpers or the entire Profile page by personal provenance.
- Workspace profile facts belong to `Membership.display_name` and `contact_email`, not the linked personal User. Admin/self name edits, SCIM reads and merges, notifications, billing contacts and tenant-visible labels use that local profile. A personal edit never propagates its values into workspace audit payloads. Keep the transactional event, local target label and request ID; do not copy the personal edit's browser/IP. Historical attribution uses the exact recorded Membership, including a tombstone for display only; it never restores authority or falls back to private User fields.
...
- Reach for `EmisarWeb.CoreComponents` before writing markup; reach for `stream/3` before assigning a list (IL-18). **One shared component per UI shape — never hand-roll a card / chip / banner / button / empty-state / page-width, and pick the right member of the stat trio (`fleet_posture`, the dashboard's private `pillar`, or `meta_strip`); there is no shared `stat`.** Worked map: `.agent/kb/rules/design-ui-shared-components.md`.
...
- **The visual language — tokens, type, brand/logo, components, the marketing↔console register split — lives in `.agent/kb/rules/design-system.md` ("The Gate"). Read it before any visual change to `emisar_web` (marketing or console).** The one accent is the emerald `brand` scale; semantic pass/pending/deny = brand/amber/rose; calm console, expressive marketing.
- Controllers / channels / MCP follow the same pattern: build a `%Subject{}` via `Subject.for_member/4` or `Subject.for_api_key/3` at the auth boundary, then pass it through. The marketing site (`controllers/marketing_html/`) is the only unauthenticated, server-rendered surface — keep it that way for SEO (see `/content-seo`).
...
- Fixtures build a real Subject when one is needed: `Fixtures.Subjects.subject_for/3` or `Fixtures.Subjects.owner_subject/1` in `test/support/fixtures/subjects.ex`.
...
- **A LiveView async helper settles to QUIESCENCE, never to a fixed number of `render_async/2` calls.** `Phoenix.LiveViewTest.render_async/2` awaits only the async processes running when it is called, so it cannot see work a `handle_async/3` completion chains afterwards. `PoliciesLive` serializes previews — a due editor only joins `preview_queue`, `start_next_preview/1` is a no-op while `preview_active` is set, each completion starts the next, and a cancelled preview keeps `preview_active` set until its own `handle_async` fires — while `load_all/1` already has a preview in flight from mount before the test makes its own edit. So a single `render_async` returned after the mount preview alone, the test's preview started too late, the editor stayed `:pending`, and the page rendered without the text the assertion wanted: the same root cause cost two separate flake investigations (`policies_performance_live_test.exs`, then `policies_live_test.exs`). Loop the helper — bounded, 10 rounds — until the view itself reports nothing left to run (no `preview_active`, empty `preview_queue`, no editor holding a `preview_timer`), and keep the assertion exactly as written: no `Process.sleep`, no retry wrapped around the assertion, no weakened comparison, no serialized suite. Force the window deterministically with a `:telemetry` handler on `[:emisar, :repo, :query]` that sleeps on the async query in non-test processes, rather than re-running the suite for a 1-in-21 failure. Sweep: a test helper that calls `render_async/2` a fixed number of times against a LiveView whose `handle_async/3` starts more work, and any per-file copy of such a helper that drifted from the looping shape.
...
Migrations are the exception to greenfield: `.agent/kb/rules/elixir-migrations-frozen.md` owns the production-applied boundary (the release runs `/app/bin/migrate` before each instance boots, once per migration), version numbering, and proportionality to real data.
