defmodule Emisar.Audit.Event.Query do
  ...
  # Every audit event type the trail can hold, grouped by the leading domain so
  # a 152-item dropdown becomes small groups an operator can scan; the Filter
  # UI renders the groups as <optgroup>s. Each row is {value, label inside its
  # group, label on its own}: "Created" under "Account" reads as "Account
  # created" where no group is shown. The two label sets are real and were
  # once two hand-kept lists of the same 152 values; one table makes "in one
  # list but not the other" unrepresentable.
  @event_types [
    {"Account",
     [
       {"account.created", "Created", "Account created"},
       {"account.updated", "Updated", "Account updated"},
       {"account.require_mfa_set", "MFA requirement changed", "MFA requirement changed"},
       {"account.require_sso_set", "SSO requirement changed", "SSO requirement changed"},
       {"account.max_grant_lifetime_set", "Maximum grant lifetime changed",
        "Maximum grant lifetime changed"},
       {"account.disabled", "Disabled", "Account disabled"},
       {"account.enabled", "Enabled", "Account enabled"},
       {"account.closed", "Closed", "Account closed"}
     ]},
    {"Runner",
     [
       {"runner.registered", "Registered", "Runner registered"},
       {"runner.connected", "Connected", "Runner connected"},
       {"runner.disconnected", "Disconnected", "Runner disconnected"},
       {"runner.disabled", "Disabled", "Runner disabled"},
       {"runner.credential_rotation_requested", "Key rotation requested",
        "Runner key rotation requested"},
       {"runner.credential_rotated", "Replacement key used", "Runner replacement key used"},
       {"runner.enabled", "Enabled", "Runner enabled"},
       {"runner.deleted", "Deleted", "Runner deleted"},
       {"runner.error", "Error", "Runner error"},
       {"runner.version_rejected", "Connection blocked: unsupported version",
        "Runner connection blocked: unsupported version"},
       {"runner.retention_swept", "Offline runners cleaned up", "Offline runners cleaned up"},
       {"dispatch_blocked_requires_attestation", "Action blocked: signature required",
        "Action blocked: signature required"}
     ]},
    {"Pack trust",
     [
       {"pack_trust_baseline_match", "Automatically trusted", "Pack automatically trusted"},
       {"pack_trust_baseline_mismatch", "Differs from published version",
        "Pack differs from published version"},
       {"pack_trust_baseline_reconciled", "Trusted after catalog update",
        "Pack trusted after catalog update"},
       {"pack_trust_review_required", "Needs review", "Pack needs review"},
       {"pack_trust_drift_detected", "New contents reported", "New pack contents reported"},
       {"pack_trust_adopted", "Hash trusted", "Pack hash trusted"},
       {"pack_trust_rejected", "Hash rejected", "Pack hash rejected"},
       {"pack_trust_revoked", "Trust revoked", "Pack trust revoked"},
       {"pack_retirement_overridden", "Retirement restriction overridden",
        "Pack retirement restriction overridden"},
       {"pack_version_deleted", "Version deleted", "Pack version deleted"},
       {"pack_deleted", "Pack deleted", "Pack deleted"},
       {"pack_retention_swept", "Unused versions cleaned up", "Unused pack versions cleaned up"},
       {"pack_retirement_swept", "Retired versions cleaned up",
        "Retired pack versions cleaned up"},
       {"dispatch_blocked_pack_untrusted", "Action blocked: pack not trusted",
        "Action blocked: pack not trusted"},
       {"dispatch_blocked_pack_retired", "Action blocked: pack version retired",
        "Action blocked: pack version retired"}
     ]},
    {"Enrollment key",
     [
       {"enrollment_key.created", "Created", "Enrollment key created"},
       {"enrollment_key.revoked", "Revoked", "Enrollment key revoked"},
       {"enrollment_key.bound", "Used for registration", "Enrollment key used for registration"}
     ]},
    {"API key",
     [
       {"api_key.created", "Created", "API key created"},
       {"api_key.rotation_requested", "Rotation requested", "API key rotation requested"},
       {"api_key.revoked", "Revoked", "API key revoked"},
       {"api_key.bound", "Used for the first time", "API key used for the first time"},
       {"api_key.auto_rotated", "Rotation started", "API key rotation started"},
       {"api_key.retired_by_rotation", "Previous key revoked after rotation",
        "Previous API key revoked after rotation"},
       {"api_key.device_grant_approved", "Agent connection approved",
        "Agent connection approved"},
       {"api_key.device_grant_denied", "Agent connection denied", "Agent connection denied"},
       {"oauth.consent_granted", "OAuth client authorized", "OAuth client authorized"},
       {"oauth.refresh_token_reused", "Refresh token reused", "OAuth refresh token reused"}
     ]},
    {"Sign-in",
     [
       {"user.signed_up", "Signed up", "User signed up"},
       {"user.signed_in", "Signed in", "User signed in"},
       {"user.signed_out", "Signed out", "User signed out"},
       {"session.account_switched", "Switched to this account", "User switched to this account"},
       {"user.sign_in_failed", "Sign-in failed", "Sign-in failed"},
       {"user.magic_link_issued", "Sign-in link created", "Sign-in link created"},
       {"user.email_confirmed", "Email confirmed", "Email confirmed"}
     ]},
    {"User security",
     [
       {"user.email_change_requested", "Email change requested", "Email change requested"},
       ...
       {"user.oidc_identity_step_up_requested", "Sign-in method confirmation requested",
        "Sign-in method confirmation requested"},
       {"user.oidc_identity_step_up_failed", "Sign-in method confirmation failed",
        "Sign-in method confirmation failed"},
       {"user.oidc_identity_step_up_rate_limited", "Sign-in method confirmation limit reached",
        "Sign-in method confirmation limit reached"},
       {"user.email_change_rate_limited", "Email change request limit reached",
        "Email change request limit reached"},
       {"user.email_changed", "Sign-in email changed", "Sign-in email changed"},
       {"user.inbox_step_up_rate_limited", "Email verification attempt limit reached",
        "Email verification attempt limit reached"},
       {"user.profile_updated", "Personal profile updated", "Personal profile updated"},
       {"user.updated_by_admin", "Profile edited by admin", "Profile edited by admin"},
       {"user.mfa_enrollment_requested", "MFA enrollment requested", "MFA enrollment requested"},
       {"user.mfa_enrollment_failed", "MFA enrollment confirmation failed",
        "MFA enrollment confirmation failed"},
       {"user.mfa_enabled", "MFA enabled", "MFA enabled"},
       {"user.mfa_disabled", "MFA disabled", "MFA disabled"},
       {"user.mfa_verified", "MFA verified", "MFA verified"},
       {"user.mfa_failed", "MFA verification failed", "MFA verification failed"},
       {"user.mfa_rate_limited", "MFA request limit reached", "MFA request limit reached"},
       {"user.mfa_recovery_code_used", "MFA recovery code used", "MFA recovery code used"},
    ...
    {"Team",
     [
       {"user.invited", "Invited", "User invited"},
       {"user.invitation_accepted", "Invitation accepted", "Invitation accepted"},
    ...
    {"Policy",
     [
       {"policy.updated", "Updated", "Policy updated"},
       {"policy.scope_deleted", "Targeted ruleset deleted", "Targeted ruleset deleted"}
     ]},
    {"Runbook",
     [
       {"runbook.created", "Created", "Runbook created"},
       {"runbook.updated", "Updated", "Runbook updated"},
       {"runbook.published", "Published", "Runbook published"},
       {"runbook.deleted", "Deleted", "Runbook deleted"},
       {"runbook.dispatched", "Execution requested", "Runbook execution requested"},
       {"runbook.execution_succeeded", "Execution succeeded", "Runbook execution succeeded"},
       {"runbook.execution_halted", "Execution halted", "Runbook execution halted"},
       {"runbook.execution_cancelled", "Execution cancelled", "Runbook execution cancelled"},
       {"runbook.stage_started", "Stage started", "Runbook stage started"},
       {"runbook.stage_succeeded", "Stage succeeded", "Runbook stage succeeded"},
       {"runbook.stage_halted", "Stage halted", "Runbook stage halted"},
       {"runbook.stage_cancelled", "Stage cancelled", "Runbook stage cancelled"},
       {"runbook.item_waiting", "Item waiting", "Runbook item waiting"},
       {"runbook.item_succeeded", "Item succeeded", "Runbook item succeeded"},
       {"runbook.item_failed", "Item failed", "Runbook item failed"},
       {"runbook.item_cancelled", "Item cancelled", "Runbook item cancelled"}
     ]},
    {"Approval",
     [
       {"approval.approved", "Granted", "Approval granted"},
       {"approval.overridden", "Requirements overridden", "Approval requirements overridden"},
       {"approval.denied", "Denied", "Approval denied"},
    ...
    {"Run",
     [
       {"run.cancel_requested", "Cancellation requested", "Run cancellation requested"},
       {"dispatch_blocked_target_unavailable", "Action request rejected",
        "Action request rejected"},
       {"action_run.success", "Succeeded", "Run succeeded"},
       {"action_run.failed", "Failed", "Run failed"},
       {"action_run.error", "Error", "Run error"},
       {"action_run.validation_failed", "Validation failed", "Run validation failed"},
       {"action_run.unknown_action", "Action not found", "Action not found on runner"},
       {"action_run.refused", "Refused", "Run refused"},
       {"action_run.cancelled", "Cancelled", "Run cancelled"},
       {"action_run.timed_out", "Timed out", "Run timed out"},
       {"action_run.denied", "Denied by policy", "Run denied by policy"},
       {"action_run.pending_approval", "Awaiting approval", "Run awaiting approval"}
     ]},
    {"SSO / Directory",
     [
       {"user.provisioned_via_sso", "User created (SSO)", "User created (SSO)"},
       {"user.provisioned_via_scim", "User created (SCIM)", "User created (SCIM)"},
       ...
       {"sso.group_mapping_updated", "Group role mapping updated",
        "SSO group role mapping updated"},
       {"sso.group_mapping_deleted", "Group role mapping deleted",
        "SSO group role mapping deleted"},
       {"sso.group_runner_access_mapping_created", "Group runner access mapping created",
        "SSO group runner access mapping created"},
       {"sso.group_runner_access_mapping_updated", "Group runner access mapping updated",
        "SSO group runner access mapping updated"},
       {"sso.group_runner_access_mapping_deleted", "Group runner access mapping deleted",
        "SSO group runner access mapping deleted"},
       {"sso.provider_configured", "Provider configured", "SSO provider configured"},
       {"sso.provider_updated", "Provider updated", "SSO provider updated"},
       {"sso.provider_deleted", "Provider deleted", "SSO provider deleted"},
       {"sso.provider_sign_in_verified", "Provider sign-in verified",
        "SSO provider sign-in verified"},
       {"sso.identity_linked", "Identity linked", "SSO identity linked"},
       {"sso.identity_unlinked", "Identity unlinked", "SSO identity unlinked"},
    ...
    {"Audit",
     [
       {"audit.exported", "Read for export", "Audit events read for export"},
       {"audit.retention_swept", "Expired events removed", "Expired audit events removed"}
     ]},
    {"Billing",
     [
    ...
    {"Emisar staff",
     [
       {"staff.account_viewed", "Viewed account", "Staff viewed account"}
     ]}
  ]

  # The flat {value, standalone label} pairs, for the console's label lookup.
  @known_event_types for {_group, options} <- @event_types,
                         {value, _short, long} <- options,
                         do: {value, long}
  ...
  # The grouped {value, label-inside-group} shape the filter renders.
  @grouped_event_types for {group, options} <- @event_types,
                           do: {group, for({value, short, _long} <- options, do: {value, short})}
