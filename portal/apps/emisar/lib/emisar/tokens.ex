defmodule Emisar.Tokens do
  ...
  | Runner enrollment key | `runner_enrollment_keys` | `rk-` | `Emisar.Runners` | `create_enrollment_key/2` | `peek_enrollment_key_by_secret/1` | `revoke_enrollment_key/2` |
  | Runner session token | `runner_tokens` | `rnrtok-` | `Emisar.Runners` | `mint_runner_token/2` | `verify_runner_token/1` | disable or delete the runner |
  | User session, magic-link, email-confirm | `user_tokens` | binary (unprefixed) | `Emisar.Auth` | `create_session_token!/5`, `request_magic_link/3`, `deliver_confirmation_instructions/1` | `fetch_user_and_token_by_session_token/1`, `verify_magic_link/4` | `delete_session_token/1`, `revoke_session/2`, `delete_all_session_tokens/1` |
