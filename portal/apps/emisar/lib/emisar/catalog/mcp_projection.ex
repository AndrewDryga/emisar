defmodule Emisar.Catalog.MCPProjection do
  ...
  Every per-pack judgment — trust, manifest match, compatibility — is computed
  from that pack's own rows, so a caller that narrows the rows upstream gets
  the same projection for the packs it kept.
  """
