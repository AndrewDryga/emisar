defmodule Emisar.Throttle do
  @moduledoc """
  Shared abuse gate over `Emisar.RateLimiter`.
  ...
  domain's MFA challenge cap (`Emisar.Auth.verify_mfa_challenge/3`), the
  ...
  def check(bucket, key, limit, window_ms) do
    if enabled?() do
      Emisar.RateLimiter.check({bucket, key}, limit, window_ms)
    else
      :ok
    end
  end

  defp enabled?, do: Emisar.Config.get_env(:emisar, :rate_limit_enabled, true)
end
