defmodule Emisar.SSO.SCIMUser do
  ...
  opaque `external_id` a directory keys every `/Users/{id}` operation on, the
  human-readable `user_name` handle, the synced `display_name`, and the
  ...
  defstruct [:external_id, :user_name, :display_name, :active]
end
