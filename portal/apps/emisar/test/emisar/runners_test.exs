defmodule Emisar.RunnersTest do
  ...
  describe "disable_runner / delete_runner — live-socket kill switch" do
    ...
    test "disabling a runner broadcasts :runner_socket_revoked to drop its live socket", %{
      subject: subject,
      runner: runner
    } do
    ...
    test "deleting a runner broadcasts :runner_socket_revoked to drop its live socket", %{
      subject: subject,
      runner: runner
    } do
  ...
  describe "signature enforcement" do
    ...
    test "runner_enforces_signatures?/2 is account-scoped" do
      ...
      account_b = Fixtures.Accounts.create_account()
      runner = Fixtures.Runners.create_runner(account_id: account_a.id, enforce_signatures: true)
  ...
  describe "connection state & presence" do
    ...
    test "list/fetch decorate online?, action_load + last heartbeat from presence", %{
      account: account,
      subject: subject
    } do
    ...
    test "status filter resolves each connection state via presence ids", %{
      account: account,
      subject: subject
    } do
  ...
  describe "mint_install_key/2" do
    ...
    test "auto-unused keys are hidden from list_auth_keys/1", %{subject: subject} do
    ...
    test "list_auth_keys status filter hides or shows revoked keys", %{subject: subject} do
  ...
  describe "verify_runner_token/1" do
    ...
    test "returns {:error, :token_invalid} for a disabled runner's token" do
      ...
      _ =
        Fixtures.Memberships.create_membership(
          account_id: account.id,
          user_id: (user = Fixtures.Users.create_user()).id,
          role: "owner"
        )
  ...
  describe "register_via_auth_key/2 with auto-generated keys" do
    ...
    test "promotes an auto-generated key to permanent on first use", %{subject: subject} do
    ...
    test "emits an auth_key.bound audit event with auto: true", %{subject: subject} do
  ...
  describe "plan-limit runner count (Billing.check_limit/2)" do
    ...
    test "an unknown/legacy plan name falls back to free limits, not a crash", %{account: account} do
      ...
      Fixtures.Accounts.create_subscription(account, "legacy-pro")
    ...
    test "member invites are uncapped — inviting past members_limit succeeds", %{
      account: account,
      subject: subject
    } do
  ...
  describe "management-write authorization (§7 denial + cross-account)" do
    ...
    test "create_runner: a viewer (no manage_runners) is refused", %{account: account} do
    ...
    test "disable_runner: a viewer (no manage_runners) is refused", %{account: account} do
      runner = Fixtures.Runners.create_runner(account_id: account.id)
    ...
    test "delete_runner: a viewer (no manage_runners) is refused", %{account: account} do
      runner = Fixtures.Runners.create_runner(account_id: account.id)
    ...
    test "create_auth_key: a viewer (no manage_auth_keys) is refused", %{account: account} do
      assert {:error, :unauthorized} =
               Runners.create_auth_key(%{reusable: true}, viewer_subject_for(account))
    end

    test "mint_install_key: a viewer (no issue_install_key) is refused", %{account: account} do
