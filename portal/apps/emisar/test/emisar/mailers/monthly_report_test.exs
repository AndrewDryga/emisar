defmodule Emisar.Mailers.MonthlyReportTest do
  ...
  defp report(overrides \\ %{}) do
    Map.merge(
      %{
        ...
        approvals: %{requested: 0, approved: 0, denied: 0, pending: 0},
        runners: 1,
        team_size: 2
      },
      Map.new(overrides)
  ...
  describe "render/4" do
    ...
    test "both bodies carry the run outcomes, the dashboard link, and the unsubscribe link" do
      ...
      assert rendered.text =~ "18 runs, dispatched to 1 runner"
    ...
    test "reports the approvals that gated work in both bodies" do
      rendered = render(%{approvals: %{requested: 31, approved: 27, denied: 4, pending: 3}})
    ...
    test "drops the approvals block from both bodies when nothing was gated" do
      rendered = render(%{approvals: %{requested: 0, approved: 0, denied: 0, pending: 0}})
    ...
    test "says runs, not runners, when policy denied every run before dispatch" do
      runs = %{total: 4, success: 0, failed: 0, denied: 4, cancelled: 0, distinct_runners: 0}
      rendered = render(%{runs: runs})

      assert rendered.text =~ "4 runs\n"
      refute rendered.text =~ "dispatched"
    end
    ...
    test "groups thousands so a busy fleet stays readable" do
      ...
      assert rendered.text =~ "12,018 runs, dispatched across 9 runners"
