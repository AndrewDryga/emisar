defmodule EmisarWeb.MarketingController do
  ...
  @pages [
    ...
    {"/docs/connect-cli-agent", :docs_connect_cli_agent, :docs_connect_cli_agent,
     "Connect a CLI agent",
     "Connect local AI clients through the emisar-mcp bridge, or call the HTTPS MCP endpoint with an emk- agent key."},
    {"/docs/connect-claude-ai", :docs_connect_claude_ai, :docs_connect_claude_ai,
     "Connect Claude.ai",
     "Add emisar to Claude.ai as a custom connector over remote MCP and OAuth."},
    {"/docs/connect-chatgpt", :docs_connect_chatgpt, :docs_connect_chatgpt, "Connect ChatGPT",
    ...
    {"/docs/signed-dispatch", :docs_signed_dispatch, :docs_signed_dispatch,
     ...
     "A customer-authorized bridge signs each request with a key the cloud never holds. An offline Ed25519 CA issues short-lived, scoped certificates; a runner trusts the CA, not every key — so onboarding, rotation, and revocation never touch the control plane."},
    ...
    {"/docs/authentication", :docs_authentication, :docs_authentication,
    ...
    {"/docs/integrations/okta", :docs_sso_okta, :docs_sso_okta, "Okta SSO & SCIM setup",
     "Connect Okta to emisar step by step, with screenshots: the OIDC web app for sign-in, the separate SCIM 2.0 app for directory provisioning, lifecycle operations, and group push."},
    {"/docs/integrations/entra", :docs_sso_entra, :docs_sso_entra,
     "Microsoft Entra SSO & SCIM setup",
     "Connect Microsoft Entra to emisar step by step, with screenshots: the app registration and oid identifier claim for sign-in, and the enterprise application with externalId remapped to objectId for provisioning."},
    {"/docs/integrations/jumpcloud", :docs_sso_jumpcloud, :docs_sso_jumpcloud,
     "JumpCloud SSO & SCIM setup",
     "Connect JumpCloud to emisar step by step, with screenshots: one custom application carrying OIDC sign-in and directory provisioning together, plus the SAML default that trips people up."},
    {"/docs/integrations/keycloak", :docs_sso_keycloak, :docs_sso_keycloak, "Keycloak SSO setup",
     "Connect Keycloak to emisar step by step, with screenshots: a confidential OpenID Connect client with PKCE — and what Keycloak's missing outbound SCIM leaves you to do."},
    {"/docs/integrations/google-workspace", :docs_sso_google_workspace,
     :docs_sso_google_workspace, "Google Workspace SSO setup",
     "Connect Google Workspace to emisar: an Internal OAuth client, emisar's locked prefilled issuer, and how to close the directory-sync gap Google's missing outbound SCIM leaves."},
    {"/docs/scim", :docs_scim, :docs_scim, "Directory sync (SCIM)",
     "SCIM 2.0 directory sync for emisar on Enterprise — provision and suspend people, map groups to roles and runner access, and integrate against the supported Users, Groups, filter, and PATCH contract."},
    ...
    {"/docs/deployment", :docs_deployment, :docs_deployment,
    ...
    {"/docs/containers", :docs_containers, :docs_containers,
     "Run the emisar runner in a container",
     "Run the emisar runner in a container: what a containerized runner can see (namespace visibility as a scoping decision), the two shared mechanics (--no-service and a persistent identity volume), and a sidecar scoped to one app."},
    {"/docs/kubernetes", :docs_kubernetes, :docs_kubernetes,
     "Kubernetes — one runner per node as a DaemonSet",
     "Run one emisar runner per Kubernetes node as a DaemonSet: a worked manifest where hostPID, hostNetwork, and read-only host mounts are the blast-radius dial, plus how to let a node action mutate deliberately."},
    {"/docs/nomad", :docs_nomad, :docs_nomad, "Nomad — one runner per node as a system job",
     "Run one emisar runner per Nomad client node as a system job: a worked HCL spec with the release binary fetched in an artifact stanza, a host volume for persistent identity, raw_exec vs docker visibility, and reusable-key + pack-credential handling."},
    {"/docs/autoscaling-fleets", :docs_autoscaling_fleets, :docs_autoscaling_fleets,
     "Autoscaling fleets — ephemeral runners on GCP, AWS, and Azure",
     "Run emisar runners on autoscaling VM groups (GCP Managed Instance Groups, AWS Auto Scaling groups, Azure scale sets): a reusable enrollment key, a zero-touch headless install for startup-script / user-data / custom-data, the fresh-identity-per-boot model, and inactive-runner retention to reap terminated instances."},
    ...
    {"/docs/agents-and-keys", :docs_agents_and_keys, :docs_agents_and_keys,
     "Manage agents & keys",
     "See the agents connected to your account, and mint, rotate, or revoke the key behind one."},
    {"/docs/bridge-upgrades", :docs_bridge_upgrades, :docs_bridge_upgrades,
    ...
    {"/docs/upgrades", :docs_upgrades, :docs_upgrades, "Upgrade runners",
