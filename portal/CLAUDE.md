...
- All crypto goes through `Emisar.Crypto` (`random_secret/1`, `hash/1`, `mint/2`, `secure_compare/2`) — never inline `:crypto.*` or `Base.url_encode64` for a secret/digest in a context. One module keeps the RNG, encoding, and hash algorithm auditable in a single place (a security product wants exactly one crypto review surface).
