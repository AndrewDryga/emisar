...
- **UI show/hide gating goes through `EmisarWeb.Permissions.can?(socket, :action)` / `gated/3`** — it maps the UI verb to a backend permission and checks the subject's permission set, so the role→permission source of truth stays in the context Authorizers and the web can't drift from it. Never branch on `current_role` / `current_membership.role` for *authorization* (`role == :owner ->` in a handler or template is the smell — gate by `can?`); rendering the role *label* is fine.
