...
- Use `Ecto.Enum` (`field :kind, Ecto.Enum, values: [:a, :b]`) for any fixed string-set field — never `:string` + a `@valid_types` list + a `validate_inclusion` in the changeset. The enum casts to atoms, validates inclusion on cast for free, and keeps the DB value as the string form. Match on the atoms (`:group`), not strings. **Two exception classes stay `:string`, each with a why-comment at the field:** a *vendor-owned value space* (`Subscription.status` — Paddle can mint statuses we've never seen, and they must persist rather than 500 the webhook; drop the inclusion list entirely there), and a *write-constrained / read-tolerant* field (`Account.plan` — the changeset's inclusion gates writes to current plans, but a stored legacy/renamed name must still LOAD and degrade gracefully; an enum raises on every fetch of such a row). Wire boundaries don't block the enum: inbound strings cast fine (changeset `cast`, query `^param`s, `update_all` sets), Jason encodes the atoms back to the same strings, and shared components normalize via `to_string` (`status_badge`, `risk_pill`).
