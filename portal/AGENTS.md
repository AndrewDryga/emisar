...
- **Product analytics goes through ONE seam — `Emisar.Analytics` — and is server-side only.** Mixpanel is the analytics tool; we ship **no** client-side SDK (strict CSP + the "no third-party trackers" brand promise on /trust). A new product event is a named builder in `Emisar.Analytics.Events` (the Mixpanel analog of `Audit.Events`/`Telemetry`), fired from the context's **post-commit** seam (never inside the txn) — never an inline `Analytics.track` at a call site, never a client `track()` call. Pageviews/funnel/identity live in `EmisarWeb.Analytics` + its `:browser`-pipeline plug; the anonymous id rides the existing session cookie (no new cookie). Always honor DNT / Global-Privacy-Control, never send run/action/audit *contents* to Mixpanel (usage events only), and disclose any new processor on `/privacy` + `/trust` + `/dpa` (one list, three templates + the marketing test). The whole thing is a no-op unless `MIXPANEL_TOKEN` is set (off in dev/test). Full tracking plan: `.agent/specs/mixpanel-analytics.md`.
