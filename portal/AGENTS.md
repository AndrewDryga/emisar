...
| `/ship-review` | Reviewing a diff before merge through the product hats **and** the Iron Laws, in parallel. The proportional product review alongside `/iron-review` (laws). |
| `/review-board` | The full pre-merge review — convenes a panel of expert hats (staff eng, domain, security, UX, UI, PM, marketing, sales) as parallel subagents, then synthesizes one ranked verdict + a prioritized fix plan (queueable into `.agent/tasks/00_todo/` for `/sweep`). Use it when correctness, security, and product risks span multiple surfaces. |
