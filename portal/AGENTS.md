...
- **Product analytics goes through ONE seam — `Emisar.Analytics` — and is server-side only.** Mixpanel is the analytics tool; we ship **no** client-side SDK (strict CSP + the "no third-party trackers" brand promise on /trust). A new product event is a named builder in `Emisar.Analytics.Events` (the Mixpanel analog of `Audit.Events`/`Telemetry`), fired from the context's **post-commit** seam (never inside the txn) — never an inline `Analytics.track` at a call site, never a client `track()` call. Pageviews/funnel/identity live in `EmisarWeb.Analytics` + its `:browser`-pipeline plug. Tracking is **cookieless**: an anonymous visitor is a daily-rotating salted IP+UA hash (`Emisar.Crypto.anonymous_visitor_id/1`, `$device:`-prefixed — the Plausible model), an identified visitor is `user.id` from the auth session; the only cookie is the functional login/CSRF session cookie — never reintroduce a session/cookie-stored analytics id. Always honor DNT / Global-Privacy-Control, never send run/action/audit *contents* to Mixpanel (usage events only), and disclose any new processor on `/privacy` + `/trust` + `/dpa` (one list, three templates + the marketing test). The whole thing is a no-op unless `MIXPANEL_TOKEN` is set (off in dev/test). Full tracking plan: `.agent/specs/mixpanel-analytics.md`.
...
| `/review-board` | The full pre-merge review — convenes a panel of expert hats (staff eng, domain, security, UX, UI, PM, marketing, sales) as parallel subagents, then synthesizes one ranked verdict + a prioritized fix plan (queueable into `.agent/TASKS.md` for `/sweep`). Supersedes running `/security-review` + `/code-review` + `/ship-review` separately. |
